...
(define (exp->string exp)
  (cases expression exp
    ...
    (self-exp () "self")
    (static-method-call-exp (obj-exp class-name method-name rands)
                     (string-append "send " (exp->string obj-exp) " " (symbol->string class-name) " " (symbol->string method-name) "(" (string-join (map exp->string rands) ",") ")"))
    (method-call-exp (obj-exp method-name rands)
                     (string-append "send " (exp->string obj-exp) " " (symbol->string method-name) "(" (string-join (map exp->string rands) ",") ")"))
	(instance-of-exp? (obj-exp symb) ; Add string representation
					   (string-append (exp->string obj-exp) "instanceof " (symbol->string symb)))
