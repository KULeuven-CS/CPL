...
; New helper function for set/get-fieldvalue
;; lookup-fieldref : exp * symbol * env -> ref 
(define lookup-fieldref
  (lambda (obj-exp field-name env)
    (let* ((obj (value-of obj-exp env)) ; use let* to let previously defined vars be usable
		  (fields (object->fields obj))
		  (classname (object->class-name obj))
		  (class (lookup-class classname))
		  (field-names (class->field-names class))
		  (new-env (extend-env field-names fields (empty-env))))
		(apply-env new-env field-name))))
